      ...
      <!--载入百度编辑器所需js文件-->
        ...
        $(function() {
            ...
            $('#f').uploadify({
                ...
                'uploader' : '{{U('Index/upload')}}',//指定服务器上传的方法
                'buttonText':'选择文件',
	...
	<body>
		...
		<div class="form-group">
			...
			<select name="category_cid" id="cate" class="form-control">
				...
				<option value="{{$v['cid']}}" tid='{{$v['type_tid']}}'  {{if value="$oldData['category_cid']==$v['cid']"}}selected{{endif}} >{{$v['_name']}}</option>
				{{endforeach}}
			</select>
		</div>
		<div class="form-group">
			...
			<select name="brand_bid" class="form-control">
				...
				<option value="{{$v['bid']}}" {{if value="$oldData['brand_bid']==$v['bid']" }}selected{{endif}}>{{$v['bname']}}</option>
				{{endforeach}}
			</select>
		</div>
		<div class="form-group">
			<label for="exampleInputEmail1">商品名称</label>
			<input id="exampleInputEmail1" class="form-control" type="text" placeholder="请输入商品名称"  name="gname" value="{{$oldData['gname']}}">
		</div>
		<div class="form-group">
			<label for="exampleInputEmail1">市场价</label>
			<input id="exampleInputEmail1" class="form-control" type="text" placeholder="请输入商品市场价" name="mprice" value="{{$oldData['mprice']}}">
		</div>
		<div class="form-group">
			<label for="exampleInputEmail1">商城价</label>
			<input id="exampleInputEmail1" class="form-control" type="text" placeholder="请输入商品商城价"  name="sprice" value="{{$oldData['sprice']}}">
		</div>
		<div class="form-group">
			<label for="exampleInputEmail1">库存</label>
			<input id="exampleInputEmail1" class="form-control" type="text" placeholder="请输入商品库存"  name="inventory" value="{{$oldData['total']}}">
		</div>
		<div class="form-group">
			<label for="exampleInputEmail1">单位</label>
			<input id="exampleInputEmail1" class="form-control" type="text" placeholder="请输入商品单位,如 件,个,双"  name="unit" value="{{$oldData['unit']}}">
		</div>
		<div class="form-group">
			<label for="exampleInputEmail1">点击次数</label>
			<input id="exampleInputEmail1" class="form-control" type="text" placeholder="请输入商品点击次数"  name="click" value="{{$oldData['click']}}">
		</div>
						...
						<tr>
							...
							<td>
								<select name="attr[{{$v['taid']}}]" id="">
									<option value="0">--请选择--</option>
									{{foreach from="$v['tavalue']" value="$va"}}
									<option value="{{$va}}" {{if value="$v['gavalue']==$va"}}selected{{endif}} >{{$va}}</option>
									{{endforeach}}
								</select>
							</td>
						</tr>
			...
			<div id="a5son">
				...
				<div class="box">
					<table class="table table-hover t2">
						<tbody>
						{{foreach from="$AttrData2" value="$v"}}
						<tr class="tr2">
							<td>{{$v['taname']}}</td>
							<td>
								<select name="spec[{{$v['taid']}}][gavalue][]">
									<option value="0">--请选择--</option>
									{{foreach from="$v['tavalue']" value="$va"}}
									<option value="{{$va}}" {{if value="$v['gavalue']==$va"}}selected{{endif}} >{{$va}}</option>
									{{endforeach}}
							...
							<td>附加价格</td>
							<td><input type="text" name="spec[{{$v['taid']}}][added][]" id="" value="{{$v['added']}}"></td>
							<td>
		...
		<div class="alert alert-success">
			...
			<div id="a1son">
				...
				<div id="uploadList">
			        <ul>
			        	<!-- 如果列表图有图片 -->
			        	{{if value="$oldData['pic_list']"}}
						<?php $img = explode(',', $oldData['pic_list']); ?>
						{{foreach from="$img" value="$v"}}
		        		<li class="img_box">
		        			<img src="{{$v}}" alt=""> 
		        			<input type="hidden" value="{{$v}}" name="pic_list[]">
		...
		<div class="alert alert-success">
			...
			<div id="a2son">
				...
				<div id="uploadList2">
			        <ul>
						<!-- 如果商品图册有图片 -->
			        	{{if value="$oldGoodsDetail['img']"}}
						<?php $img = explode(',', $oldGoodsDetail['img']); ?>
						{{foreach from="$img" value="$v"}}
		        		<li class="img_box">
		        			<img src="{{$v}}" alt=""> 
		...
		<div class="alert alert-success dv">
			...
			<div id="a3son">
				<!--调用百度编辑器-->
				<script id="editor" type="text/plain" style="width:100%;height:500px;" name="detail">{{$oldGoodsDetail['detail']}}</script>
				<script>
		...
		<div class="alert alert-success dv">
			...
			<div id="a6son">
				<!--调用百度编辑器-->
				<script id="editor2" type="text/plain" style="width:100%;height:500px;" name="service">{{$oldGoodsDetail['service']}}</script>
				<script>
