			...
			<div class="right">
						...
						<dd>
							<div class="price">
								<?php 
									...
									$temp[$k] = 0;
								 ?>
								<a <?php if($param[$k] == 0): ?>class="now"<?php endif ?> href="{{U('GoodsList/bidshow',array('bid'=>$_GET['bid'],'param'=>implode('_',$temp)))}}">不限</a>
								{{foreach from="$v['value']" value='$va'}}
								<?php 
									$temp[$k] = $va['gaid'];
								 ?>
								<a <?php if($param[$k] == $va['gaid']): ?>class="now"<?php endif ?> href="{{U('GoodsList/bidshow',array('bid'=>$_GET['bid'],'param'=>implode('_',$temp)))}}">{{$va['gavalue']}}</a>
								{{endforeach}}
							</div>
						</dd>
					</dl>
				...
				<div id="content">
					...
					<!-- 调取数据库里的图片 -->
						...
						<li class="ul_li1">
							<div class="img1">
								<a href="{{U('Content/index',array('gid'=>$v['gid']))}}"><img src="{{$v['pic_list'][0]}}" alt="" /></a>
							</div>
							<div class="img_change">
								{{foreach from="$v['pic_list']" value="$vv"}}
								<a href="{{U('Content/index',array('gid'=>$v['gid']))}}"><img src="{{$vv}}" alt="" /></a>
								{{endforeach}}
