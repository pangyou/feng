	...
	<!-- action="javascript:void(0);" -->
		...
		<table class="table table-hover">
			...
			<tr class="active">
				<td>{{$v['glid']}}</td>
				<!-- 调取控制器数据 -->
				<foreach name="v['combine']" item="va">
				<td>{{$va}}</td>
				</foreach>
				<td>{{$v['inventory']}}</td>
				<td>{{$v['number']}}</td>
				<td>
					<a href="{{:U('edit',array('glid'=>$v['glid'],'gid'=>$_GET['gid']))}}" class="btn btn-sm btn-warning">修改</a>
					<a href="javascript:if(confirm('确定删除吗?')) location.href='{{:U('del',array('glid'=>$v['glid'],'gid'=>$_GET['gid']))}}'" class="btn btn-sm btn-danger">删除</a>
				</td>
			</tr>
			<?php } ?>
			<tr class="active">
				...
				<td>
					<select name="gavalue[]" class="form-control">
						...
						<!-- 颜色,尺寸 -->
						<foreach name="v['gaid']" key='k' item='vv'>
