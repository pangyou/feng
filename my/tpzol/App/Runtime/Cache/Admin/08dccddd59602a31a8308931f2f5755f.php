...
<html>
	...
	<body>
		...
		<div class="form-group">
			...
			<input id="exampleInputEmail1" class="form-control" type="text" placeholder="请输入品牌名称" value="<?php echo ($oldData['bname']); ?>" name="bname">
		</div>
		<div class="form-group dv2">
			<label for="exampleInputEmail1" class="label1">是否热门</label>
			<input type="radio" class="radio" <?php if($oldData['hot'] == '是'): ?>checked<?php endif; ?> value="是" name="hot" id="">
			<span>是</span> 
			<input type="radio" class="radio"<?php if($oldData['hot'] == '否'): ?>checked<?php endif; ?> value="否" name="hot" id="">
			<span>否</span>
		</div>
		<div class="form-group">
			...
			<div id="img_box">
				<!-- 如果这篇文章有图片 -->
				<?php if($oldData['logo']): ?><img src="<?php echo ($oldData['logo']); ?>" style="width:250px;height:auto;" alt="" id="img">
				<a id="close_img" href="javascript:;">x</a>
				<input type="hidden" name="logo" value="<?php echo ($oldData['logo']); ?>">
				<?php else: ?>
				<!-- 没有图片显示上传表单 -->
				<input id="exampleInputEmail1" type="file" name="logo"><?php endif; ?>
			</div>
		</div>
