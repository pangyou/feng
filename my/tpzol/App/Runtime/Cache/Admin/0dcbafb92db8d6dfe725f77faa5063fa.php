...
<html>
	...
	<body>
		<div class="dv1 form-group">
			您当前的位置是:<span>首页 > 类型管理 > 修改类型</span>
			<a href="Javascript:window.history.go(-1)" class="return">
				返回上一步
			</a>
		</div>
		<form onsubmit="return hd_submit(this,'<?php echo U('edit');?>','<?php echo U('index');?>')">
		<div class="alert alert-success">修改类型</div>
		<div class="form-group">
			<label for="exampleInputEmail1">类型名称</label>
			<input id="exampleInputEmail1" class="form-control" type="text" placeholder="请输入分类名称" value="<?php echo ($oldData['tname']); ?>"  name="tname">
		</div>
