...
<html>
	...
	<body>
		...
		<div class="form-group">
			...
			<input id="exampleInputEmail1" class="form-control" type="text" placeholder="请输入属性名称"  value="<?php echo ($oldData['taname']); ?>" name="taname">
		</div>
		<div class="form-group dv2">
			<label for="exampleInputEmail1" class="label1">属性类别</label>
			<input type="radio" class="radio" value="属性"<?php if($oldData['class'] == '属性'): ?>checked<?php endif; ?> name="class" id="">
			<span>属性</span> 
			<input type="radio" class="radio" value="规格" <?php if($oldData['class'] == '规格'): ?>checked<?php endif; ?> name="class" id="">
			<span>规格</span>
		</div>
		<div class="form-group"><span></span> 
			<label for="exampleInputEmail1">属性值</label>
			<textarea name="tavalue" rows="5" cols=""  class="form-control" placeholder="添加多个属性值请按照,分开"><?php echo ($oldData['tavalue']); ?></textarea>
		</div>
		<input type="hidden" name="taid" value="<?php echo ($_GET['taid']); ?>">
