	...
	<body>
		...
		<table class="table table-hover">
			...
			<?php if(is_array($data)): foreach($data as $key=>$v): ?><tr>
				...
				<td>
					<a href="<?php echo U('edit',array('taid'=>$v['taid'],'tid'=>$_GET['tid']));?>" class="btn btn-sm btn-warning">修改</a>
					<a href="javascript:if(confirm('确定删除吗?')) location.href='<?php echo U('del',array('taid'=>$v['taid']));?>'" class="btn btn-sm btn-danger">删除</a>
				</td>
			</tr><?php endforeach; endif; ?>
		</table>
		<a href="<?php echo U('add',array('tid'=>$_GET['tid']));?>" class="btn btn-primary btn-block">添加属性</a>
	</body>
</html>